...
router.get('/save', authenticate, requireAuth, requireRole(['SEEKER']), async (req: AuthenticatedRequest, res) => {
  try {
...
router.post('/save', authenticate, requireAuth, requireRole(['SEEKER']), async (req: AuthenticatedRequest, res) => {
  try {
...
router.delete('/save', authenticate, requireAuth, requireRole(['SEEKER']), async (req: AuthenticatedRequest, res) => {
  try {
...
router.get('/', async (req, res) => {
  try {
    ...
    const salaryMin = req.query.salaryMin ? parseInt(req.query.salaryMin as string, 10) : null;
...
router.post('/', authenticate, requireAuth, requireRole(['EMPLOYER']), async (req: AuthenticatedRequest, res) => {
  try {
...
router.get('/homepage', async (req, res) => {
  try {
    const jobsCount = await prisma.job.count({ where: { status: 'ACTIVE' } });
    const companiesCount = await prisma.employerProfile.count();
    const seekersCount = await prisma.jobSeekerProfile.count();
...
router.put('/:id', authenticate, requireAuth, requireRole(['EMPLOYER']), async (req: AuthenticatedRequest, res) => {
  try {
    const employerProfile = await prisma.employerProfile.findUnique({
      where: { userId: req.user!.userId },
    });
...
router.delete('/:id', authenticate, requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const job = await prisma.job.findUnique({
      where: { id: req.params.id },
    });
