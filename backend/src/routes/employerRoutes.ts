...
router.put('/profile', authenticate, requireAuth, requireRole(['EMPLOYER']), async (req: AuthenticatedRequest, res) => {
  try {
    const {
      companyName,
      logoUrl,
...
router.get('/analytics', authenticate, requireAuth, requireRole(['EMPLOYER']), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const statusFunnel = await prisma.application.groupBy({
      by: ['status'],
      where: {
        job: { employerId: employer.id },
      },
...
router.get('/candidates', authenticate, requireAuth, requireRole(['EMPLOYER']), async (req: AuthenticatedRequest, res) => {
  try {
